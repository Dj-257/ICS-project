...
void dijkstraMaze(int maze[N][N]) {
    ...
    dist[0][0] = 0;  // Starting point distance
    int x = 0, y = 0;
    ...
    while (1) {
        ...
        if (minDist == INF || (x == N - 1 && y == N - 1)) break;
    }
    ...
    if (dist[N - 1][N - 1] == INF) {
        ...
        printf("Shortest path from start to end has length: %d\n", dist[N - 1][N - 1]);
        
        // Trace the path from the end to the start
        int pathX = N - 1, pathY = N - 1;
        ...
        // Mark starting point as 3 and ending point as 4
        maze[0][0] = 3;
        maze[N - 1][N - 1] = 4;
